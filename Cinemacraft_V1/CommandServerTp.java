...
public class CommandServerTp extends CommandBase
{
    ...
    public void processCommand(ICommandSender par1ICommandSender, String[] par2ArrayOfStr)
    {
        ...
        /*// here we also try to update all those who may be currently following us (e.g. camera should update projector, and projector spectators)
        if (Vars.isCamera && Vars.getCameraByName(getCommandSenderAsPlayer(par1ICommandSender).getEntityName()) == Vars.cameraViewPortValue && !Vars.projectorName.equals(""))
        {
        	//this is the camera that is currently being followed by the projector
        	Minecraft.getMinecraft().thePlayer.sendChatMessage(String.format("@updatetp %s", Vars.projectorName));      	
        }
        // do this only if the original command was issued by a projector, otherwise, we get infinite loop
        else if (getCommandSenderAsPlayer(par1ICommandSender).getEntityName().equals(Vars.projectorName) && Vars.isProjector && Vars.spectatorsFollowingProjector.size() > 0)
        {
    		...
    		for (int i = 0; i < Vars.spectatorsFollowingProjector.size(); i++) {
    			...
    			Minecraft.getMinecraft().getIntegratedServer().executeCommand("/tp " + spectatorUsername + " " + Vars.projectorName);
    		}
        }*/
    }
