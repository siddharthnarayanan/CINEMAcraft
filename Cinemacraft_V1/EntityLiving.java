...
public abstract class EntityLiving extends Entity
{
    ...
    public String getTexture()
    {
    	...
    	{
        	System.out.println("getTexture: p=" + Vars.isProjector + " c=" + Vars.isCamera + " a=" + Vars.isActor + " s=" + Vars.isSpectator + " getEntityName=" + this.getEntityName() + " containsKey=" + Vars.playerToRoles.containsKey(this.getEntityName()) + "\n");
    	}*/
    	
    	//IMPORTANT! Spectators should not be included in playerToRoles hashMap 
    	
    	// if this is server it should be always invisible
    	if (this instanceof EntityPlayer && Vars.playerToRoles.containsKey(this.getEntityName()) && Vars.playerToRoles.get(this.getEntityName()).equals("server"))
    	{
    		//This is a spectator without assigned role, so we make them invisible to active participants
    		//Vars.isServer sees them but we don't care about that
    		return "/mob/invisible.png";
    	}
    	
    	if (this instanceof EntityPlayer && (Vars.isProjector || Vars.isCamera || Vars.isActor) && 
    			(!Vars.playerToRoles.containsKey(this.getEntityName()) || Vars.playerToRoles.get(this.getEntityName()).equals("spectator")))
    	{
    		//This is a spectator without assigned role, so we make them invisible to active participants
    		//Vars.isServer sees them but we don't care about that
    		return "/mob/invisible.png";
    	...
    	//if this is a spectator, it should not see other spectators unless they are unlatched from the main camera and have HUD on
    	if (Vars.isSpectator &&
    			(!Vars.playerToRoles.containsKey(this.getEntityName()) || Vars.playerToRoles.get(this.getEntityName()).equals("spectator")))
    	{
    		if ((Vars.followProjector > 0 && Vars.followProjector < 3) || Vars.projectorView)
    		{
        		//The spectator has either disabled HUD or is latched onto the projector, so we make the other spectator
    			//we are currently rendering invisible 
        		return "/mob/invisible.png";    			
    		}
    	}
    	
    	if (Vars.playerToRoles.containsKey(this.getEntityName()))
    	{
        	//if I am the projector and am rendering a camera
        	if (Vars.isProjector && Vars.playerToRoles.get(this.getEntityName()).equals("camera"))
        	{
        		//System.out.println("I am the projector and am rendering a camera invisible\n");
        		return "/mob/invisible.png";
        	}
        	//if I am a camera and am rendering the projector -- DISABLED, using the one below
        	/*else if (Vars.isCamera && Vars.playerToRoles.get(this.getEntityName()).equals("projector"))
        	{
        		//System.out.println("I am a camera and am rendering the projector\n");
        		return "/mob/invisible.png";
        	}*/
        	else if (!Vars.isProjector) // was (Vars.isActor || Vars.isCamera), but we may want spectators also to see cameras
        	{
            	//Players should see custom views of regular cameras and not see projector at all unless projector is in the
        		//freeroam mode, in which case it is treated as yet another camera (2 skins, one for on, one for off)
        		//Same goes for cameras seeing other cameras (so that they have a better understanding what angles are already covered)
        		//System.out.println(this.getEntityName() + " " + Vars.currentCamera + "\n");
        		if (Vars.playerToRoles.get(this.getEntityName()).equals("camera"))
        		{
        			// we make cameras invisible when spectators are observing action through the cam view
        			if (Vars.isSpectator && Vars.followProjector > 0 && Vars.followProjector < 3)
        				return "/mob/invisible.png";
        			if (this.getEntityName().equals(Vars.currentCamera))
        			{
        				//System.out.println("cameraON!");
        				return "/mob/cameraOn.png";
        			}
        			else
        			{
        				return "/mob/cameraOff.png";
        			}
        		}
        		else if (Vars.playerToRoles.get(this.getEntityName()).equals("projector"))
        		{
        			// we make projector invisible when spectators are observing action through cam view
        			if (Vars.isSpectator && Vars.followProjector > 0 && Vars.followProjector < 3)
        				return "/mob/invisible.png";
        			if (Vars.currentCamera.equals("freeroam"))
        			{
        				// projector is visible when in freeroam mode (it basically means it acts as an independent camera)
    ...
    public void onUpdate()
    {
        ...
        {
        	// MIRRORCRAFT (Ico 20160406): using kinectPosition to filter out renderYawOffset that is used in strafing and moving back and forward
            ...
            this.renderYawOffset = this.rotationYaw - (Vars.kinectPosition == 0 ? var10 : 0);
