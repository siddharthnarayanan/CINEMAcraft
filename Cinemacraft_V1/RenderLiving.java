...
public class RenderLiving extends Render
{
    ...
    protected void passSpecialRender(EntityLiving par1EntityLiving, double par2, double par4, double par6)
    {
        if (Minecraft.isGuiEnabled() && par1EntityLiving != this.renderManager.livingPlayer && !par1EntityLiving.func_98034_c(Minecraft.getMinecraft().thePlayer) && (par1EntityLiving.func_94059_bO() || par1EntityLiving.func_94056_bM() && par1EntityLiving == this.renderManager.field_96451_i))
        {
            ...
            if (var10 < (double)(var12 * var12))
            {
                ...
                if (par1EntityLiving.isSneaking() || Vars.projectorView) {
                    FontRenderer var14 = this.getFontRendererFromRenderManager();
                    GL11.glPushMatrix();
                    //removes the name entirely from view when either sneaking or in camera man view 'h' key
    ...
    {
    	//System.out.println("------------ " + par1EntityLiving.getEntityName() + " " + Vars.playerToRoles.get(par1EntityLiving.getEntityName()));
    	
    	// do not draw a label above a spectator for active participants (actors, camera, projector) to minimize distraction
    	if (Vars.playerToRoles.get(par1EntityLiving.getEntityName()) == null && !Vars.isSpectator)
    		return;
    	// likewise do not draw label above active participants for spectators, so that they don't know their usernames
    	// also never draw label over the server client ("server" role)
    	else if (Vars.playerToRoles.get(par1EntityLiving.getEntityName()) != null && (Vars.isSpectator || Vars.playerToRoles.get(par1EntityLiving.getEntityName()).equals("server")))
    		return;
