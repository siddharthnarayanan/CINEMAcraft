...
public class RenderPlayer extends RenderLiving
{
    ...
    {
    	// called when looking at the player from external point of view
    	// if we are a camera or projector looking at ourselves, make this match camera view
    	// This, however, is not necessary, as cameras should never look outside their first person view
    	// otherwise the video feed would be... well, not camera-like anymore
    	/*System.out.println(par1EntityPlayer.getEntityName() + " " + Vars.playerToRoles.get(par1EntityPlayer.getEntityName()));
    	if (Vars.playerToRoles.get(par1EntityPlayer.getEntityName()).equals("projector") ||
    			Vars.playerToRoles.get(par1EntityPlayer.getEntityName()).equals("camera"))
    	{
    		System.out.println("YEP");
    		this.loadDownloadableImageTexture("null", par1EntityPlayer.getTexture());
    	}
    	else
    	{*/
    ...
    protected void renderSpecials(EntityPlayer par1EntityPlayer, float par2)
    {
        ...
        {
            GL11.glPushMatrix();
            //part that matches Kinect-induced rotations of the arm that is holding the item
            
            this.modelBipedMain.bipedHead.postRender(0.0625F);
            
            //if (player != null)
            //	GL11.glRotatef(player.RArmRot * (180F / (float) Math.PI), player.RArmOrthoX, 0.0F, player.RArmOrthoZ);
            
            float var5;
            
            if (var4.getItem().itemID < 4096)
            {
        ...
        if (this.loadDownloadableImageTexture(par1EntityPlayer.cloakUrl, (String)null) && !par1EntityPlayer.isInvisible() && !par1EntityPlayer.getHideCape())
        {
            ...
            this.modelBipedMain.renderCloak(0.0625F, (Entity)par1EntityPlayer);
            GL11.glPopMatrix();
        }
        ...
        if (var21 != null)
        {
            ...
            // part that matches Kinect-induced rotations of the arm that is holding the item
            // this is now disabled as there is a global displace above
            //GL11.glTranslatef(offset_x, offset_y, offset_z);
            if (player != null)
            	GL11.glRotatef(player.RArmRot * (180F / (float) Math.PI), player.RArmOrthoX, 0.0F, player.RArmOrthoZ);
