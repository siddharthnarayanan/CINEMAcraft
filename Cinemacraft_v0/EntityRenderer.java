...
public class EntityRenderer
{
    ...
    private void renderHand(float par1, int par2)
    {
        if (this.debugViewDirection <= 0)
        {
            ...
            if (!Vars.projectorView && this.mc.gameSettings.thirdPersonView == 0 && !this.mc.renderViewEntity.isPlayerSleeping() && !this.mc.gameSettings.hideGUI && !this.mc.playerController.enableEverythingIsScrewedUpMode())
            {
    ...
    public void updateCameraAndRender(float par1)
    {
        ...
        if (!this.mc.skipRenderWorld)
        {
            ...
            {
            	//THIS IS NOT TRUE: + is hidden even with this commented out, so where is the crosshair getting hidden then?
            	//hides the + on the screen           	
                //this.mc.ingameGUI.renderGameOverlay(par1, false, var16, var17);
            }
        }        
    }
    ...
    public void renderWorld(float par1, long par2)
    {
        ...
        for (int var13 = 0; var13 < 2; ++var13)
        {
            ...
            if (this.debugViewDirection == 0)
            {
                ...
                if (!Vars.projectorView && this.mc.objectMouseOver != null && var4.isInsideOfMaterial(Material.water) && var4 instanceof EntityPlayer && !this.mc.gameSettings.hideGUI)
                {
            ...
            if (!Vars.projectorView && this.cameraZoom == 1.0D && var4 instanceof EntityPlayer && !this.mc.gameSettings.hideGUI && this.mc.objectMouseOver != null && !var4.isInsideOfMaterial(Material.water))
            {
                var17 = (EntityPlayer)var4;
                GL11.glDisable(GL11.GL_ALPHA_TEST);
                this.mc.mcProfiler.endStartSection("outline");
