...
public abstract class Render
{
    ...
    {
    	boolean isPlayer = par1Entity instanceof EntityPlayer;
    	//System.out.println("-----"+par1Entity.getEntityName()+" "+this.shadowSize);
    	if ( !isPlayer || (Vars.playerToRoles.containsKey(par1Entity.getEntityName()) && Vars.playerToRoles.get(par1Entity.getEntityName()).equals("actor")))
	    {
	        ...
	        // ICO 20160429 -- implemented Kinect offset
            OperaCraftPlayer player = Vars.getOperaCraftPlayer(par1Entity.getEntityName());
            float offset_x = 0;
            float offset_y = 0;
            float offset_z = 0;
        	if (player != null && Vars.kinectPosition == 2)
        	{
            	//TODO ICO: shadow needs to be calculated per axes (as per former teleportation code)
				double yawRadZ = par1Entity.rotationYaw * Math.PI / 180.0f;
				double yawRadX = (par1Entity.rotationYaw + 90) * Math.PI / 180.0f;
				if (Vars.kinectMirror == 1)
				{
					//offset_x = -player.TorsoX;
    				//offset_y = -(float)player.TorsoY;
    				//offset_z = -player.TorsoZ;
    				offset_x = (float)(Math.sin(yawRadZ)*player.TorsoZ - Math.sin(yawRadX)*player.TorsoX);
    				offset_z = -(float)(Math.cos(yawRadZ)*player.TorsoZ - Math.cos(yawRadX)*player.TorsoX);				
				}
				else
				{
					//offset_x = -player.TorsoX;
	        ...
	        double var24 = par2 + offset_x - var35;
	        double var26 = par4 - var14;
	        double var28 = par6 + offset_z - var16;
	        ...
	        for (int var31 = var18; var31 <= var19; ++var31)
	        {
	            for (int var32 = var20; var32 <= var21; ++var32)
	            {
	                for (int var33 = var22; var33 <= var23; ++var33)
	                {
	                    ...
	                    {
	                        this.renderShadowOnBlock(Block.blocksList[var34], par2 + offset_x, par4 + (double)par1Entity.getShadowSize(), par6 + offset_z, var31, var32, var33, par8, var11, var24, var26 + (double)par1Entity.getShadowSize(), var28);
	                    }
	                }
	            }
	        }
